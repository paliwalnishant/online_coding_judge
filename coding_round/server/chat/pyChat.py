...
def server_options_window(master):
    ...
    server_options_go(50018, top)  #sonakshi
...
class Server (threading.Thread):
    ...
    def run(self):
        ...
        s.setsockopt(socket.SOL_SOCKET,socket.SO_REUSEADDR, 1)     #sonakshi
        self.port=50018
        try:
        	#s.bind(('', self.port))
        	s.bind(('', self.port))
...
if len(sys.argv) > 1 and sys.argv[1] == "-cli":
    ...
    statusConnect.set("Enable chat with users...")
    clientType = 1
    ...
    connecter = Button(root, textvariable=statusConnect,
                       command=lambda: connects(1))   #sonakshi
    connecter.pack()
