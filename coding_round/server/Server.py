...
def handleclient(conn,addr):
    ...
    while True:
	    ...
	    if(choice=='1'):
		    ...
		    db1 = MySQLdb.connect("localhost","root","sona","major" )
		    cursor1 = db1.cursor()
		    ...
		    results = cursor1.fetchall()
		    prevruns=''
		    prevruns=prevruns+'id\tquestio\tlanguage\tresult\ttime\n'
		    for row in results:
			    prevruns=prevruns+str(row[0])+'\t'+str(row[1])+'\t'+str(row[2])+'\t'+str(row[3])+'\t'+str(row[4])+'\n'
		    db1.close()
		    conn.send(prevruns)	    
		    conn.recv(1)
	    elif(choice=='4'):
...
db = MySQLdb.connect("localhost","root","sona","major" )
cursor = db.cursor()
subid=0
contestflag=0
usersflag=0
